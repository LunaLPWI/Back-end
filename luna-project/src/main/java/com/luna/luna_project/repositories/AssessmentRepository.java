...
public interface AssessmentRepository extends JpaRepository<Assessment, Long> {
    List<Assessment> findAllByEstablishment_Id(Long establishmentId);
    ...
    @Query("SELECT a FROM Assessment a " +
            ...
            "AND s.startDateTime < :currentDateTime " +
            "AND a.rating IS NULL")
    List<Assessment> findAssessmentsByClientIdAndPastScheduling(
            @Param("clientId") Long clientId,
            @Param("currentDateTime") LocalDateTime currentDateTime);

    Optional<Assessment> findByScheduling(Scheduling scheduling1);

    Optional<Assessment> findByScheduling_Id(Long id);
}
